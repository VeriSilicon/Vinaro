...
typedef enum {
    IRQ_GROUP_MODE = 0, /**< An interrupt handler handles a group of gpio
                           containing multiple gpio  */
    IRQ_IND_MODE, /**< An interrupt handler handles only a gpio irq*/
} GpioIrqHandleMode;
...
typedef struct GpioHwConfig {
    ...
    GpioIrqHandleMode irq_mode; /**< Interrupt handle mode from soc config */
} GpioHwConfig;
...
typedef struct GpioOperations {
    ...
    int (*io_irq)(const GpioDevice *device, unsigned char io_idx,
                  uint8_t b_irq);
    int (*io_trigger)(const GpioDevice *device, unsigned char io_idx,
                      GpioIrqTrigger tgr);
    ...
    int (*irq_clear)(const GpioDevice *device, unsigned char io_idx);
 ...
 * @note The API should be called by BSP
...
/**
 * @brief Clear interrupt status for an input GPIO pin
 * @param gpio The gpio pin which provides the interrupt trigger
 * @return Return result
 * @retval VSD_SUCCESS for succeed, others for failure
 */
 ...
 * @param irq_id The interrupt id follow a irq handler
 */
